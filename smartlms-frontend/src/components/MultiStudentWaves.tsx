  ...
  useEffect(() => {
    ...
    const studentIdsParam = selectedStudentIds.length > 0 ? selectedStudentIds.join(',') : undefined;
      ...
      .then(res => {
        ...
        const chartData = timeline.map((min: number, idx: number) => {
          ...
          students.forEach((s: any) => {
            if (s && s.student_name) {
              const waveArray = Array.isArray(s.wave) ? s.wave : [];
              point[s.student_name] = waveArray[idx] || 0;
              
              // Store lapses as hidden properties for the custom tooltip
              const lapseWave = Array.isArray(s.lapse_wave) ? s.lapse_wave : [];
              const tabWave = Array.isArray(s.tab_wave) ? s.tab_wave : [];
              
              point[`${s.student_name}_lapses`] = lapseWave[idx] || 0;
              point[`${s.student_name}_tabs`] = tabWave[idx] || 0;
            }
          });
          return point;
        });
      ...
      .catch(err => {
        ...
        setData({ chartData: [], studentNames: [] });
      })
